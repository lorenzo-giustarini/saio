...
export class LinuxInstaller implements IInstaller {
  constructor(private pkgMgr: LinuxPackageManager) {}
  ...
  async installCli(spec: {
    ...
    if (spec.aptPackage) {
      return await this.pkgMgr.install(spec.aptPackage)
    }
    return { ok: false, error: `no install method for ${spec.cliName} on Linux` }
  }
}
